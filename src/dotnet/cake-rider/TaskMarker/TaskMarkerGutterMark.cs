...
{
    public TaskMarkerGutterMark()
        : base(CakeIcons.CakeGutterMark)
    {
    }
    ...
    {
        if (highlighter.UserData is not TaskMarkerHighlighting data)
        {
            yield break;
        }
        ...
        {
            if (configuration.TypeId != "CakeFrosting")
            {
                continue;
            }

            if (FileSystemPath.TryParse(configuration.ProjectFilePath) != project.ProjectFileLocation.ToNativeFileSystemPath())
            {
                continue;
            }

            if (!cakeFrostingProjectsModel.RunConfigurationTaskNames.TryGetValue(configuration, out var taskName) || taskName != data.TaskName)
            {
                continue;
            }
        ...
        if (configurationHost.AvailableTemplates.Count != 0)
        {
            ...
            {
                if (template.TypeId != "CakeFrosting")
                {
                    continue;
                }

                if (configurationExistsForExecutor.Contains(template.Executor.Id))
                {
                    continue;
                }
                ...
                foreach (var runnableProject in runnableProjectFactory.CreateRunnableProjects(project).Where(p => p.Kind == CommonRunnableProjectKinds.DotNetCore))
                {
                    if (template.CompatibleRunnableProjectKinds.Contains(runnableProject.Kind.Name))
                    {
                        ...
                        {
                            new RunConfigurationTemplateEntry(RunConfigurationTemplateKey.Name, name),
                            new RunConfigurationTemplateEntry(RunConfigurationTemplateKey.ProjectFilePath, runnableProject.ProjectFilePath),
                            new RunConfigurationTemplateEntry(RunConfigurationTemplateKey.StaticMethodName, data.TaskName),
                        };
