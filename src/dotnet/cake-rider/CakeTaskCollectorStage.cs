...
public sealed class CakeTaskCollectorStage : CSharpDaemonStageBase
{
    ...
    {
        if (processKind != DaemonProcessKind.SOLUTION_ANALYSIS) return null;
...
public sealed class CakeTaskCollectorProcess : CSharpDaemonStageProcessBase
{
    ...
    public CakeTaskCollectorProcess([NotNull] IDaemonProcess process, [NotNull] ICSharpFile file, CakeFrostingProjectsHost host) : base(process, file)
    {
        _host = host;
    }
