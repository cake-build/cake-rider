...
public static class Extensions
{
    ...
    {
        if (!klass.IsCakeFrostingTask()) throw new ArgumentException("Given type is not a cake task", nameof(klass));

        var taskNameAttribute = klass.GetAttributeInstances(CakePredefinedType.TaskNameAttributeFqn, false).SingleOrDefault();
        return taskNameAttribute != null ? (string)taskNameAttribute.PositionParameter(0).ConstantValue.Value : klass.ShortName;
    }
}
